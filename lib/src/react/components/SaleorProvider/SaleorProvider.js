...
const SaleorProvider = ({ apolloConfig, config, children, appversion, appplatform }) => {
    ...
    react_1.useEffect(() => {
        const manager = new __1.SaleorManager(config, apolloConfig, appversion, appplatform);
        getSaleorApiAndClient(manager);
    }, []);
    if (client && context) {
        return (<context_1.SaleorContext.Provider value={context}>
        <client_1.ApolloProvider client={client}>{children}</client_1.ApolloProvider>
      </context_1.SaleorContext.Provider>);
    }
    return null;
};
